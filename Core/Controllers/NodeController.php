...
class Core_Controllers_NodeController extends Core_Model_Node
{
    ...
    {            
        ...
        try
        {
            ...
            else            
            {
                ...
                $methodName=Core::covertStringToMethod($FieldName."_addSingleFilter");
        ...
        if(count($errorsArray)==0)
        {
            ...
            if(Core::countArray($UnqueFieldSetAttributes)>0)
            {
                foreach ($UnqueFieldSetAttributes as $UnqueFieldSet) 
                {
                    ...
                    $db->setTable($this->_tableName); 
                    $db->addField("count(".$this->_tableName.".$this->_primaryKey)");
                    $UnqueFieldSet_list= Core::covertStringToArray($UnqueFieldSet);
                    ...
                    if($existingCount>0)
                    {
                        $errorsArray[$fieldName]=" Value must be Unque for Combination :: (".Core::covertArrayToString($UnqueFieldSet_list_label,",").") ";
                    }
                }
            }
                ...
                foreach($fileattribute_list as $key)
                {
                    ...
                    if(Core::keyInArray($key,$filesData))
                    {
                            ...
                            if($columnnamedata['name']!="")
                            {
                                ...
                                $db->addWhere("core_cms_image_settings.id in('".Core::covertArrayToString(Core::covertStringToArray($filePath[$key]['imagesizeid']),"','")."')");
                                $db->buildSelect();
    ...
    public function multiEditSaveAction() 
    {
            ...
            $method=Core::covertStringToMethod($this->_nodeName."_beforeMRADataValidate"); 
            if(Core::methodExists($this, $method))
            {
                $errors=$this->$method();
            }            
            ...
            {
                $method=Core::covertStringToMethod($this->_nodeName."_afterMRADataValidate"); 
                if(Core::methodExists($this, $method))
                {
                    $errors=$this->$method;
                }
    ...
    public function finalAction()
    {
        ...
        $method=Core::covertStringToMethod($this->_nodeName."_beforeFinal");
        if(Core::methodExists($this, $method))
        {
           $errorsArray=$this->$method();
        }
        if(Core::countArray($errorsArray)==0)
        {
            ...
            $nodeSave->save();
            $method=Core::covertStringToMethod($this->_nodeName."_afterFinal");
            if(Core::methodExists($this, $method))
            {
               $errorsArray=$this->$method();
            }
    ...
    {
        $method=Core::covertStringToMethod($this->_nodeName."_beforeunFinal");
        if(Core::methodExists($this, $method))
        {
           $errorsArray=$this->$method();
        }
        if(Core::countArray($errorsArray)==0)
        {
            ...
            $nodeSave->save();
            $method=Core::covertStringToMethod($this->_nodeName."_afterunFinal");
            if(Core::methodExists($this, $method))
            {
               $errorsArray=$this->$method();
            }
